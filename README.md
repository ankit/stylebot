...
**Pin the dev build to the store id (maintainers).** Put the store build's public key — Chrome Web Store developer dashboard → the item → Package → "Public key" — in a git-ignored `.extension-key` file at the repo root, or in `STYLEBOT_EXTENSION_KEY`. Development builds (`yarn watch`, `yarn build:dev`, `yarn dev:chrome`, `yarn chrome:plain`) then emit it as the manifest `key`, so the unpacked extension has the store id and the existing OAuth client accepts it. Production builds never include it. Chrome refuses to load two extensions with one id, so disable the store install in any profile that loads the dev build (`yarn dev:chrome` uses its own profile, so it is unaffected).
