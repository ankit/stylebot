...
stylebot.page = {
  ...
  toggleLivePreview: function () {
    ...
    stylebot.chrome.saveOption(
      'livePreviewPage',
      stylebot.page.cache.livePreview
    );
  },
  ...
  displaySyntaxError: function (error, setCursor) {
    if (!this.cache.marker) {
      ...
      this.cache.marker = this.cache.editor
        .getSession()
        .addMarker(range, 'stylebot_warning', 'line');
    }
  ...
  resize: function () {
    $('#stylebot-page-editor').css(
      'height',
      $(WidgetUI.SELECTORS.stylebot).height() -
        this.BOTTOM_EDITOR_PADDING +
        'px'
    );
