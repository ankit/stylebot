...
const getCachedToken = (): Promise<AccessToken | null> =>
  new Promise(resolve => {
...
const setCachedToken = (token: AccessToken, expiresIn: number): Promise<void> =>
  new Promise(resolve => {
    const expiresAt = Date.now() + expiresIn * 1000 - EXPIRY_SKEW_MS;
    chrome.storage.local.set({ [CACHE_KEY]: { token, expiresAt } }, () =>
      resolve()
    );
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.identity.launchWebAuthFlow(
      ...
      responseURL => {
        ...
        if (lastError) {
          reject(syncError(lastError.message ?? 'Authorization failure', 'auth'));
          return;
        }
