...
stylebot.widget = {
  ...
  create: function() {
    ...
    WidgetUI.createOption(
      WidgetUI.createButtonSet(
        ['Basic', 'Advanced'],
        'stylebot-mode',
        0,
        self.toggleMode
  ...
  attachListeners: function() {
    var lastBt = $('#stylebot-main-buttons')
      .find('button')
      .last();

    // Shift + TAB on first accordion sets focus to last button
    $(WidgetUI.SELECTORS.accordion).bind(
      'keydown',
      { lastBt: lastBt },
      function(e) {
  ...
  updateHeight: function() {
    ...
    if (headerHeight != 0) headerHeight -= 42;

    var optionsHeight = $('#stylebot-widget-options').height();
    var newHeight = windowHeight - (optionsHeight + headerHeight);

    if (stylebot.options.mode === 'Advanced') self.advanced.resize(newHeight);
    else self.basic.resize(newHeight);
  },
   ...
   */
  reset: function() {
    if (stylebot.options.mode === 'Advanced') stylebot.widget.advanced.reset();
    else stylebot.widget.basic.reset();
  },
  ...
  togglePosition: function(e) {
    ...
    var pos;
    if (e) pos = $el.data('position');
    else pos = stylebot.options.position;

    if (pos === 'Left') {
      pos = 'Right';
      $el
        .removeClass('stylebot-arrow-right')
        .addClass('stylebot-arrow-left')
        .attr('title', 'Move stylebot to the left');
    } else {
      pos = 'Left';
      $el
        .removeClass('stylebot-arrow-left')
  ...
  toggleMode: function(e) {
    if (e) stylebot.options.mode = $(e.target).html();
    else if (stylebot.options.mode === 'Advanced')
      stylebot.options.mode = 'Basic';
    else stylebot.options.mode = 'Advanced';
  ...
  showSelectorDropdown: function() {
    ...
    for (var selector in stylebot.style.rules) {
      ...
      var li = $('<li>', {
        ...
        .hover(function(e) {
          if (stylebot.selectionStatus) return true;
    ...
    if (!any) $('<li>', { html: 'No CSS selectors edited' }).appendTo(dropdown);
  ...
  setSelector: function(value) {
    ...
    if (value === 'Select an element') value = 'Click to edit the CSS Selector';
