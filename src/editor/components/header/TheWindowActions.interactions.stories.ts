...
const meta: Meta = {
  title: 'Editor/Interactions/Window actions',
...
export const DockFromOptionsMenu = editor(withRule, {
...
export const DockShortcuts = editor(withRule, {
  play: async ({ canvasElement }) => {
    const store = storeOf(canvasElement);
...
export const AdjustPageLayout = editor(withRule, {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const store = storeOf(canvasElement);
...
export const AppearanceMenu = editor(withRule, {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const store = storeOf(canvasElement);
...
export const CloseButton = editor(withRule, {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
    const store = storeOf(canvasElement);
...
export const EscapeClosesEditor = editor(withRule, {
  play: async ({ canvasElement }) => {
    await pressKey('Escape');
    await waitFor(() => expect(content(canvasElement)).toBeNull());
  },
...
export const EscapeClosesMenuBeforeEditor = editor(withRule, {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
...
export const HelpDialogViaShortcut = editor(withRule, {
  play: async ({ canvasElement }) => {
    const store = storeOf(canvasElement);
...
export const HelpDialogViaMenu = editor(withRule, {
  play: async ({ canvasElement }) => {
    const canvas = within(canvasElement);
