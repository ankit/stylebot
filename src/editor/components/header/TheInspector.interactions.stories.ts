...
export const ButtonAndShortcutToggle = editor(
  { activeSelector: 'h1', css: 'h1 { color: red; }' },
  {
...
export const PickSetsSelector = editor(
  { inspecting: true },
  {
    play: async ({ canvasElement }) => {
...
export const ArrowKeysClimbAncestors = editor(
  { inspecting: true },
  {
    page: INSPECT_PAGE,
    play: async ({ canvasElement }) => {
      ...
      // Hovering the link shows its selector and offers its parent as the
      // next step up.
      ...
      // Selecting stops inspecting and applies the climbed-to selector,
      // not the <a>'s.
      await pressKey('ArrowUp');
      await pressKey('Enter');

      await expect(store.state.inspecting).toBe(false);
      await waitFor(() => expect(selectorChip(canvasElement)).toMatch(/\bp$/));
...
export const ClickOnIframeSelectsIt = editor(
  { inspecting: true },
  {
    page: IFRAME_PAGE,
    play: async ({ canvasElement }) => {
      ...
      await waitFor(() =>
        expect(selectorChip(canvasElement)).toMatch(/iframe/)
      );
    },
  }
);
