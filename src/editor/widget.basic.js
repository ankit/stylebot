...
stylebot.widget.basic = {
  ...
  create: function () {
    ...
    for (var i = 0; i < len; i++) {
      WidgetUI.createAccordionHeader(this.groups[i].name).appendTo(
  ...
  createUIForControl: function (control) {
    ...
    switch (control.type) {
      case 'size':
        control_el = ui.createSizeControl(control.id).appendTo($controlSet);
        break;

      case 'multi-size':
        control_el = ui.createMultiSizeControl(control).appendTo($controlSet);
        break;

      case 'color':
        control_el = ui.createTextField(
          control.id,
          10,
          null,
          Events.onTextFieldKeyUp
        );

        ui.createColorPicker(control_el, $('#stylebot-container')).appendTo(
          $controlSet
        );
      ...
      case 'toggle':
        control_el = ui
          .createToggleButton('Hide', control.id, control.value)
          .appendTo($controlSet);
        break;

      case 'segmented':
        control_el = ui.createSegmentedControl(control).appendTo($controlSet);
        break;
      ...
      case 'border-style':
        control_el = ui.createBorderStyleControl(control).appendTo($controlSet);
        break;
    }
