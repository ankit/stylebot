...
- `yarn chrome:plain` — dev build + launch a flag-free Chrome (no debugging port) on the dev profile, for flows Google sign-in blocks in debugged browsers, e.g. Google Drive sync; one-time manual Load unpacked, no hot reload
