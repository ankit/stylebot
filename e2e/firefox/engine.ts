...
export class FirefoxEngine implements Engine {
  ...
  async launch(
    ...
    return firefox.launchPersistentContext(userDataDir, {
      ...options,
      args: [
        `--start-debugger-server=${this.rdpPort}`,
        // Same reason as the Chromium engine's --window-size: the default headless
        // window is too small for the editor's header controls.
        '--width=1400',
        '--height=900',
      ],
      firefoxUserPrefs: {
